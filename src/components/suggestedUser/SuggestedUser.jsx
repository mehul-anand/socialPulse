...
export const SuggestedUser = () => {
  ...
  const usersNotFollowedByLoginUser = allUsers.filter((user) => {
    return (
      ...
      !user.followers.some(
        (follower) => follower.username === loginUser.username
      )
    );
  });
  ...
  return (
    <div className="sticky top-0 m-3">
      <h1 className="flex flex-col font-bold text-xl p-2 text-gray-600">
        Rising Creators
      </h1>
      <div>
        {suggestedUsers?.length > 0 ? (
          suggestedUsers?.map((user) => (
            ...
            >
              ...
              <button
                className="bg-emerald-400 hover:bg-emerald-500 text-white font-bold py-2 px-2 rounded-lg h-10 mr-2 shadow shadow-blue-500/40 hover:shadow-indigo-500/40"
                onClick={() =>
                  handleBtnsClick(400, followUserHandler, user?._id, token)
                }
              >
                Follow
              </button>
          ...
          <h1 className="font-medium text-lg text-center mt-10">
            Oops! No user found
          </h1>
        )}
      </div>
